...
class User {
    ...
    public function CategoryList($sort_order = array('category'), $filter_by = array()) {
    	try {
    		$dbh = PDOConnect::init();
    		...
    		// Include the current?
    		if (Kit::GetParam('withCurrent', $filter_by, _INT, 0) != 0) {
    			$SQL .= ' OR categoryid = :categoryid ';
    			$params['categoryid'] = Kit::GetParam('withCurrent', $filter_by, _INT);
    		}
    		...
    		if (is_array($sort_order))
    			...
    			foreach ($results as $row) {
    				...
    				$res['categoryid'] = Kit::ValidateParam($row['categoryID'], _INT);
    				$res['category'] = Kit::ValidateParam($row['category'], _STRING);
    				$res['width'] = Kit::ValidateParam($row['width'], _INT);
    				$res['height'] = Kit::ValidateParam($row['height'], _INT);
    				$res['intended_width'] = Kit::ValidateParam($row['intended_width'], _INT);
    				$res['intended_height'] = Kit::ValidateParam($row['intended_height'], _INT);
